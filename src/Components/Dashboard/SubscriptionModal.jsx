...
const SubscriptionCheckoutForm = ({ refetch, onClose, userEmail }) => {
  ...
  useEffect(() => {
    ...
    axiosSecure
      .post("/create-payment-intent", { price: SUBSCRIPTION_PRICE })
      .then((res) => {
    ...
    if (confirmError) {
      ...
      if (paymentIntent.status === "succeeded") {
        ...
        try {
          ...
          // Upgrade User
          const res = await axiosSecure.patch(`/users/premium/${userEmail}`);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="alert alert-warning shadow-sm mb-4">
        <div>
          <span>
            Subscribe for <strong>{SUBSCRIPTION_PRICE} BDT</strong> to unlock
            unlimited reports!
          </span>
        </div>
      </div>
