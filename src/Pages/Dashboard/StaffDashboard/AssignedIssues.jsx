                ...
                {filteredIssues.map(issue => (
                    <div key={issue._id} className="card bg-base-100 shadow-xl border border-base-200 hover:shadow-2xl transition-all duration-300">
                        <figure className="h-48 overflow-hidden relative">
                            <img src={issue.image || issue.img} alt={issue.title} className="w-full h-full object-cover transition-transform duration-500 hover:scale-110" />
                            <div className="absolute top-2 right-2">
                                <div className={`badge ${issue.priority === 'high' ? 'badge-error' : issue.priority === 'medium' ? 'badge-warning' : 'badge-info'} badge-sm shadow-md`}>
                                    {issue.priority}
                                </div>
                            </div>
                        </figure>
                        <div className="card-body">
                            ...
                            <p className="text-xs text-gray-400 absolute top-4 left-4 bg-white/80 px-2 py-0.5 rounded backdrop-blur-sm">{issue.category}</p>
                            
                            <p className="text-sm text-gray-500 flex items-center gap-1 mt-1">
                                <span className="font-semibold">Loc:</span> {issue.location}
                            </p>
                            <p className="text-sm line-clamp-2 mt-2 text-gray-600">{issue.description}</p>
                            
                            <div className="divider my-2"></div>

                            <div className="card-actions justify-between items-center">
                                <span className={`badge ${issue.status === 'pending' ? 'badge-warning' : issue.status === 'in-progress' ? 'badge-info' : 'badge-success'} p-3`}>
                                    {issue.status}
                                </span>
                                
                                <div className="dropdown dropdown-end dropdown-top">
                                    <label tabIndex={0} className="btn btn-sm btn-primary text-white">Update Status</label>
                                    <ul tabIndex={0} className="dropdown-content z-[1] menu p-2 shadow-lg bg-base-100 rounded-box w-52 border border-gray-100">
