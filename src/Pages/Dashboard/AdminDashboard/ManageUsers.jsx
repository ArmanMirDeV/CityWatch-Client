...
const ManageUsers = () => {
    ...
    const handleBlockUser = (user) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                const res = await axiosPublic.patch(`/users/block/${user.email}`, { isBlocked: true });
    ...
    const handleUnblockUser = (user) => {
         Swal.fire({
            ...
            if (result.isConfirmed) {
                const res = await axiosPublic.patch(`/users/block/${user.email}`, { isBlocked: false });
                if (res.data.modifiedCount > 0) {
                    refetch();
                        ...
                        {users.map((user, index) => (
                            <tr key={user._id}>
                                ...
                                <td>
                                    { user.role !== 'admin' && (
                                        user.isBlocked ? 
                                        <button onClick={() => handleUnblockUser(user)} className="btn btn-sm btn-success text-white" title="Unblock User">
                                            <FaCheckCircle /> Unblock
                                        </button> :
                                        <button onClick={() => handleBlockUser(user)} className="btn btn-sm btn-error text-white" title="Block User">
                                            <FaBan /> Block
                                        </button>
                                    )}
                                </td>
